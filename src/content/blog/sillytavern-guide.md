...
- **World Info / lorebooks.** This is the killer feature. A lorebook is a set of entries that get injected into the prompt only when their keywords come up — so a sprawling world's lore is available on demand without permanently eating your context window. It's the closest thing to giving the AI a reference book it consults as needed. (It's the same idea as [NovelAI](/blog/sillytavern-vs-novelai)'s Lorebook, but free and on the model of your choice — see [SillyTavern vs NovelAI](/blog/sillytavern-vs-novelai).)
