...
RoleForge isn't alone in betting on that separation. In our [MythEngine first look](/blog/mythengine-first-look), the open-beta developer described building "a referee the AI can't overrule" as the actual hard problem — a different implementation, the same underlying wager that narration and ground truth need to be two separate jobs.
