...
faq:
  ...
  - q: "Does RoleForge have better memory than Janitor AI?"
    a: "By design, yes — though we haven't tested RoleForge hands-on ourselves, since it's closed alpha. Its published architecture stores characters, inventory, and world state in a SQL database that the AI narrates from but cannot override, so nothing should drift across a long campaign. Janitor AI keeps no memory beyond the current context window — every session's continuity depends entirely on what still fits in the chat."
  ...
  - q: "Is RoleForge rated higher than Janitor AI?"
    a: "RoleForge doesn't currently carry a numeric rating, because Arcanum's policy is not to score pre-release software — it's in closed alpha, we haven't tested it ourselves, and a rating will be assigned at public release. Janitor AI is rated 4/5 in our full review. Judge RoleForge on its architecture preview rather than a side-by-side score for now."
  ...
  - q: "Should I use RoleForge, Janitor AI, or both?"
    a: "Different itches, so using both isn't unusual. Use Janitor AI for character-driven conversation with a specific persona. Use RoleForge (once you're off the waitlist) if what you actually want is a solo campaign with real stakes and a world that's designed not to forget. Neither replaces the other."
---
...
| **Rating** | **Unrated — closed alpha, not yet tested by us** ([preview](/blog/roleforge-preview)) | **4/5** ([full review](/blog/janitor-ai)) |
...
[RoleForge](/clients/roleforge) answers a completely different question: "can I trust this world to remember what happened?" Per its published architecture, it separates mechanical truth from narrative rendering — an SQL database holds hit points, inventory, faction reputation, and NPC relationship scores, and the AI's only job is to narrate what the database says is true; it can't override the rules, because the rules aren't its job. We haven't tested RoleForge hands-on yet, since it's closed alpha, but that architectural choice is covered in full in our [RoleForge preview](/blog/roleforge-preview).
...
RoleForge was built specifically to solve this — per its own architecture description, log off for a week, come back, and your inventory should be exactly as you left it, and the NPC you insulted three sessions ago should still hold the grudge, because that grudge lives as a database entry rather than something the AI has to remember. We haven't verified that hands-on, since RoleForge remains closed alpha, but for anyone whose real frustration with character-chat platforms was "nothing carries forward," that's the entire premise RoleForge is built around.
...
Per our policy of not scoring pre-release software, RoleForge currently carries no numeric rating, and we haven't tested it ourselves; the analysis in our [preview](/blog/roleforge-preview) is based on its public architecture rather than hands-on play, and a score will be assigned at public release. If RoleForge's access gate is the deciding factor for you right now, Janitor AI (or a structured [Arcanum Original](/arcanum-games)) is available today with no wait.
...
**Choose RoleForge if** you want a solo campaign designed to remember what happened, are willing to apply for alpha access, and are comfortable with a world built to push back rather than bend to a clever line of dialogue. **[RoleForge preview](/blog/roleforge-preview)** (unrated — closed alpha, not yet tested by us).
...
Different itches, so using both isn't unusual. Use Janitor AI for character-driven conversation with a specific persona. Use RoleForge (once you're off the waitlist) if what you actually want is a solo campaign with real stakes and a world that's designed not to forget. Neither replaces the other.
